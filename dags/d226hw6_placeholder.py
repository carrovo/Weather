...
def extract(city, longitude, latitude):
    ...
    print(f"Extracting only one day: {date_to_fetch}")
    print(f"Next day (for incremental window): {next_day_of_date_to_fetch}")
    
    save_weather_data(city, latitude, longitude, date_to_fetch, date_to_fetch, file_path)
...
def load(file_path, database, schema, target_table):
    ...
    try:
        ...
        cur.execute(f"""
            ...
            WHERE date >= TO_DATE('{date_to_fetch}') 
              AND date < TO_DATE('{next_day_of_date_to_fetch}')
        """)
...
with DAG(
    ...
    LATITUDE = Variable.get("LATITUDE_1")            # TO DO: Use Variable.get to get latitude value from Airflow Variables
    LONGITUDE = Variable.get("LONGITUDE_1")          # TO DO: Use Variable.get to get longitude value from Airflow Variables
    CITY = "San Jose"               # TO DO: Set the city name to your coordinates' city name
    
    database = "USER_DB_CRICKET"           # TO DO: Set the database name  
    schema = "RAW"
    target_table = "WEATHER_DAILY_INCREMENTAL"       # TO DO: Set the target table name
